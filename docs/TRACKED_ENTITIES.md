...
| 2 | abundance | tbl_abundances | fact | analysis_entity_id, taxon_id | no | abundance, taxonomy |
| 3 | relative_dating | tbl_relative_dates | fact | analysis_entity_id, relative_age_id | no | dating |
| 4 | geochronology | tbl_geochronology | fact | analysis_entity_id, lab_number | no | dating |
...
|---|--------|-------------|---------|--------------|--------|----------|
| 5 | sample_description | tbl_sample_descriptions | fact | physical_sample_id, sample_description_type_id | sample | no |
| 6 | sample_coordinate | tbl_sample_coordinates | fact | physical_sample_id, coordinate_method_dimension_id | sample | no |
| 7 | sample_alt_ref | tbl_sample_alt_refs | fact | physical_sample_id, alt_ref_type_id, alt_ref | sample | no |
| 8 | sample_dimension | tbl_sample_dimensions | fact | physical_sample_id, dimension_id, method_id | sample | no |
| 9 | abundance_property | tbl_abundance_properties | fact | abundance_id, property_type_id, property_value | abundance | no |
...
| # | Entity | Target Table | TM Role | Business Key | Required | Domains |
|---|--------|-------------|---------|--------------|----------|---------|
| 10 | location | tbl_locations | lookup | location_type_id, location_name | yes | core |
| 11 | site | tbl_sites | lookup | site_name | yes | core |
| 12 | sample_group | tbl_sample_groups | lookup | site_id, sample_group_name | yes | core |
| 13 | dataset | tbl_datasets | lookup | dataset_name | yes | core |
| 14 | master_dataset | tbl_dataset_masters | lookup | master_name | no | core, provenance |
| 15 | project | tbl_projects | lookup | project_name | no | core, provenance |
| 16 | citation | tbl_biblio | lookup | title, year, authors | no | provenance |
| 17 | chronology | tbl_chronologies | lookup | chronology_name | no | dating |
| 18 | dating_material | tbl_dating_material | lookup | dating_material_name | no | dating |
| 19 | relative_ages | tbl_relative_ages | lookup | relative_age_name | no | dating |
| 20 | dating_lab | tbl_dating_labs | lookup | international_lab_id | no | dating |
| 21 | contact | tbl_contacts | lookup | *(undefined)* | no | contacts |
| 22 | feature | tbl_features | lookup | feature_type_id, feature_name | no | core, excavation |
| 23 | taxa_common_names | tbl_taxa_common_names | lookup | taxon_id, common_name | no | taxonomy |

> **Gaps:**
> - `contact` has no `identity_columns` in the target model. Candidates: `[last_name, first_name]` or `[email]`. Needs decision.
> - `taxa_common_names` may be better classified as an aggregate child of `taxa_tree_master`.
...
| # | Entity | Target Table | TM Role | Business Key | Required | Domains |
|---|--------|-------------|---------|--------------|----------|---------|
| 24 | location_type | tbl_location_types | classifier | location_type | no | core |
| 25 | site_type_group | tbl_site_type_groups | classifier | site_type_group | no | core |
| 26 | site_type | tbl_site_types | classifier | site_type | no | core |
| 27 | sample_type | tbl_sample_types | classifier | type_name | yes | core |
| 28 | sample_description_type | tbl_sample_description_types | classifier | type_name | no | sample-metadata |
| 29 | dimension | tbl_dimensions | classifier | dimension_abbrev | no | spatial |
| 30 | coordinate_method_dimension | tbl_coordinate_method_dimensions | classifier | method_id, dimension_id | no | spatial |
| 31 | alt_ref_type | tbl_alt_ref_types | classifier | alt_ref_type | no | sample-metadata |
| 32 | method | tbl_methods | classifier | method_name | yes | core |
| 33 | method_group | tbl_method_groups | classifier | group_name | no | core |
| 34 | abundance_element | tbl_abundance_elements | classifier | element_name | no | abundance |
| 35 | abundance_element_group | *(none)* | classifier | abundance_element_group_name | no | abundance |
| 36 | modification_type | tbl_modification_types | classifier | modification_type_name | no | abundance |
...
> - `taxa_tree_master` is the canonical SEAD taxonomy. While SEAD-administered, new taxa are periodically added through a curated process.
...
| 45 | analysis_entity | tbl_analysis_entities | bridge | *(physical_sample_id, dataset_id)* | sample, dataset | yes |
| 46 | abundance_modification | tbl_abundance_modifications | bridge | abundance_id, modification_type_id | abundance, modification_type | no |
| 47 | abundance_ident_level | tbl_abundance_ident_levels | bridge | abundance_id, identification_level_id | abundance, identification_level | no |
| 48 | dataset_contact | tbl_dataset_contacts | bridge | dataset_id, contact_id, contact_type_id | dataset, contact, contact_type | no |
| 49 | sample_feature | tbl_physical_sample_features | bridge | physical_sample_id, feature_id | sample, feature | no |

> **Note on analysis_entity.** The target model classifies `analysis_entity` as a bridge (sample ↔ dataset), but it also serves as the observation context — the parent entity for abundance, geochronology, and relative-dating records. For SIMS, it functions as a **provider-owned entity** with full identity tracking and a composite business key derived from its FK columns.
...
| abundance | abundance_property | 1 child entity type |
| analysis_entity | *(abundance, relative_dating, geochronology)* | Observation children; see note below |
| relative_dating | *(none in target model)* | Leaf entity |
| geochronology | *(none in target model)* | Leaf entity |

> **analysis_entity as aggregate root.** Although classified as a bridge, analysis_entity acts as an aggregate root for observation data. Its "children" (abundance, relative_dating, geochronology) are themselves fact entities with independent business keys. Whether these are value objects or independent entities affects change detection scope — see Open Questions §7.
...
| tbl_analysis_entities | sample | dataset | — | Also observation context (§2.4) |
...
| contact | reconcile-fuzzy | *(undefined — see §7)* | People have variable name formats |
| feature | reconcile-exact | [feature_type_id, feature_name] | Per site-context; low collision risk |
| taxa_common_names | reconcile-exact | [taxon_id, common_name] | Per-taxon; deterministic |
...
1. **Contact business key.** `contact` has no `identity_columns` in the target model. Should the business key be `[last_name, first_name]`, `[email]`, or a composite? This affects C1 completeness.

2. **Analysis entity classification.** The target model classifies `analysis_entity` as a bridge, but it serves as an aggregate root for observations. Should SIMS treat it as provider-owned with full UUID tracking?

3. **Taxa common names ownership.** Is `taxa_common_names` an independent entity or a value object within the `taxa_tree_master` aggregate?
...
5. **Observation aggregate scope.** Are abundance, relative_dating, and geochronology independent aggregate roots (each with their own UUID), or value objects within an analysis_entity aggregate? This affects both identity tracking granularity and change detection scope.
